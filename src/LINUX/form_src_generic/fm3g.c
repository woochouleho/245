...
int fm3g_checkWrite(int eid, request * wp, int argc, char **argv)
{
	...
	if(!strcmp(name, "fm3g-enable-dis")) {
		...
		return 0;
	}else if(!strcmp(name, "fm3g-auth-mschap")) {
		if(p->auth==PPP_AUTH_MSCHAP) boaWrite(wp, "selected");
		return 0;
	}else if(!strcmp(name, "fm3g-auth-mschapv2")) {
		if(p->auth==PPP_AUTH_MSCHAPV2) boaWrite(wp, "selected");
		return 0;
