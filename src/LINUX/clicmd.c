...
typedef enum {
	...
	CMD_DISABLE_RIP,
	CMD_ENABLE_RIP,
	CMD_IGP_PROTOCOL_SETTING,
	CMD_ADD_IGP_RIP,
	CMD_DELETE_IGP_RIP,
	CMD_ADD_IGP_OSPF,
	CMD_DELETE_IGP_OSPF,
	CMD_ADD_IGP_BGP_NETWORK,
	CMD_DELETE_IGP_BGP_NETWORK,
	CMD_ADD_IGP_BGP_NEIGHBOUR,
	CMD_DELETE_IGP_BGP_NEIGHBOUR,
	CMD_ADD_IGP_ISIS,
	CMD_DELETE_IGP_ISIS,
	CMD_SHOW_IGP_PROTOCOL,
	CMD_SHOW_IGP_RIP_TBL,
	CMD_SHOW_IGP_OSPF_TBL,
	CMD_SHOW_IGP_BGP_TBL,
	CMD_SHOW_IGP_ISIS_TBL,
...
#if defined(CONFIG_EXT_SWITCH) || defined(CONFIG_RTL_IGMP_SNOOPING) || defined(CONFIG_BRIDGE_IGMP_SNOOPING)
...
static char string13[260];
static char string14[260];
static char string15[260];
static char string16[260];
static char string17[260];
static char string18[260];
static char string19[260];
...
static struct arg_table cmdArgTable[] = {
	...
	{STRING13,(void *)string13,"string13"},
	{STRING14,(void *)string14,"string14"},
	{STRING15,(void *)string15,"string15"},
	{STRING16,(void *)string16,"string16"},
	{STRING17,(void *)string17,"string17"},
	{STRING18,(void *)string18,"string18"},
	{STRING19,(void *)string19,"string19"},
...
static void cmdModifyWanInterface(int cmdType, struct cmd_arg *pCmdArg);
...
static void cmdIgpProtocol(int cmdType, struct cmd_arg *pCmdArg);
static void cmdSetIgp(int cmdType, struct cmd_arg *pCmdArg);
static void cmdShowIgpProtocol(int cmdType, struct cmd_arg *pCmdArg);
static void cmdShowIgpTbl(int cmdType, struct cmd_arg *pCmdArg);
#endif
...
static char iana_help[] = "\"on\" or \"off\". \"on\" means request Address .Default is \"off\"";
static char iapd_help[] = "\"on\" or \"off\". \"on\" means request prefix .Default is \"off\"";
static char ipv6_help[]="Format ####:####:####:####:####:####:####:####";
static char ctype_help[] = "The OR value of 0x01 (TR069), 0x02 (INTERNET), 0x04 (OTHER), 0x08 (VOICE). Default is 0x04 (OTHER)";
static char ipProtocol_help[] = "The OR value of 0x01 (ipv4), 0x02 (ipv6), 0x03 (ipv4/ipv6). Default is 0x01 (ipv4)";
...
static char ma_help[] = "Multicast Allowed. \"disable\" or \"enable\".";
static char rc_help[] = "Robust Count.";
static char lmqc_help[] = "Last Member Query Count.";
static char qi_help[] = "Query Interval (seconds).";
static char qri_help[] = "Query Response Interval(*100ms).";
static char gld_help[] = "Group Leave Delay(ms).";
#endif
...
#ifdef CONFIG_USER_QUAGGA
static char igp_protocol_help[] = "IGP Protocol must be NONE/RIP/OSPF/BGP/ISIS";
static char igp_isis_area_help[] = "area: 1-255";
static char igp_bgp_as_help[]="as: 1-65535";
static char igp_bgp_remoteas_help[]="remoteas: 1-65535";
static char igp_isis_level_help[] = "level: -1/-1-2/-2";
#endif
...
static struct cmd_arg wan_modify_args[] = {
	{"interface",		STRING1,		ARG_VALUE,			wan_interfacename_help},
#ifdef ENABLE_802_1Q
...
#endif
#ifdef CONFIG_IGMPPROXY_MULTIWAN
	{"igmpproxy",		STRING2,		ARG_BOTH_OPTIONAL, 		enable_help},
#endif
	{0,0,0,0}
};
...
#endif
#ifdef CONFIG_IPV6
	{"ipProtocol",	NUMBER6,		ARG_BOTH_OPTIONAL,		ipProtocol_help},
#endif
#ifdef CONFIG_USER_RTK_WAN_CTYPE
	{"ctype",		NUMBER3,		ARG_BOTH_OPTIONAL,		ctype_help},
#endif
...
END_SELECT_ARG
	{"ipv4Dns1",		STRING7,		ARG_BOTH_OPTIONAL,		dns_help},
	{"ipv4Dns2",		STRING8,		ARG_BOTH_OPTIONAL,		dns_help},
#ifdef CONFIG_IPV6
START_SELECT_ARG
	{"slaac",		STRING6,		ARG_NAME_OPTIONAL,		0},
DETACH_SELECT_ARG
	{"ipv6Addr",		STRING11,		ARG_BOTH_OPTIONAL,		ipv6_help},
	{"ipv6PrefixLen",	NUMBER7,	ARG_BOTH_OPTIONAL,		0},
	{"ipv6Gateway",		STRING12,		ARG_BOTH_OPTIONAL,		ipv6_help},
END_SELECT_ARG
	{"iana",		STRING13,		ARG_BOTH_OPTIONAL,		iana_help},
	{"iapd",		STRING14,		ARG_BOTH_OPTIONAL,		iapd_help},
	{"ipv6Dns1",		STRING15,		ARG_BOTH_OPTIONAL,		dns_help},
	{"ipv6Dns2",		STRING16,		ARG_BOTH_OPTIONAL,		dns_help},
#endif
...
END_SELECT_ARG
#ifdef CONFIG_IGMPPROXY_MULTIWAN
	{"igmpproxy",		STRING15,		ARG_BOTH_OPTIONAL, enable_help},
#endif
	{0,0,0,0}
};
...
#ifdef CONFIG_IPV6
START_SELECT_ARG
	{"slaac",		STRING13,		ARG_NAME_OPTIONAL,		0},
DETACH_SELECT_ARG
	{"ipv6Addr",		STRING14,		ARG_BOTH_OPTIONAL,		ipv6_help},
	{"ipv6PrefixLen",	NUMBER7,	ARG_BOTH_OPTIONAL,		0},
	{"ipv6Gateway", STRING15,		ARG_BOTH_OPTIONAL,		ipv6_help},
END_SELECT_ARG
#endif
#ifdef CONFIG_IPV6
	{"iana",		STRING16,		ARG_BOTH_OPTIONAL,		iana_help},
	{"iapd",		STRING17,		ARG_BOTH_OPTIONAL,		iapd_help},
	{"ipv6Dns1",		STRING18,		ARG_BOTH_OPTIONAL,		dns_help},
	{"ipv6Dns2",		STRING19,		ARG_BOTH_OPTIONAL,		dns_help},
#endif
#ifdef DEFAULT_GATEWAY_V1
...
END_SELECT_ARG
#ifdef CONFIG_IGMPPROXY_MULTIWAN
	{"igmpproxy",		STRING20,		ARG_BOTH_OPTIONAL,		enable_help},
#endif
...
#ifdef CONFIG_USER_QUAGGA
static struct cmd_arg igp_rip_add_args[] = {
	{"interface",		STRING1,		ARG_VALUE,			rip_interfacename_help},
	{"recvmode",		STRING2,		ARG_BOTH,			rip_recv_mode_help},
	{"sendmode",		STRING3,		ARG_BOTH,			rip_send_mode_help},
	{0,0,0,0}
...
static struct cmd_arg igp_ospf_add_args[] = {
	{"ipaddr",		IPADDR1,		ARG_BOTH,		0},
	{"mask",		IPMASK1,		ARG_BOTH,		0},
	{0,0,0,0}
};

static struct  cmd_arg igp_bgp_network_add_args[] = {
	{"as",		STRING1,		ARG_BOTH,		igp_bgp_as_help},
	{"router-id",	IPADDR1,		ARG_BOTH,		0},
	{"ipaddr",		IPADDR2,		ARG_BOTH_OPTIONAL,		0},
	{"mask",		IPMASK1,		ARG_BOTH_OPTIONAL,		0},
	{0,0,0,0}
};

static struct  cmd_arg igp_bgp_neighbour_add_args[] = {
	{"as",		STRING1,		ARG_BOTH,		igp_bgp_as_help},
	{"router-id",	IPADDR1,		ARG_BOTH,		0},
	{"ipaddr",		IPADDR2,		ARG_BOTH_OPTIONAL,		0},
	{"remote-as",	STRING2,		ARG_BOTH_OPTIONAL,		igp_bgp_remoteas_help},
	{0,0,0,0}
};

static struct  cmd_arg igp_isis_add_args[] = {
	{"area",		NUMBER1,		ARG_BOTH,		igp_isis_area_help},
	{"level",		STRING1,		ARG_BOTH,		igp_isis_level_help},
	{"interface",	STRING2,		ARG_VALUE,		rip_interfacename_help},
	{0,0,0,0}
};

static struct cmd_arg igp_protocol_setting_args[]={
	{"protocol", 	STRING1,		ARG_VALUE,			igp_protocol_help},
	{0,0,0,0}
};
#endif
...
//	{"autopvc",	0, 	0,	channel_autopvc_nodes,	0,					0,						listNextLevelNodes,		"Auto-pvc search"},
	...
	{"modify",	END, 	CMD_MODIFY_WAN_INTERFACE,	0,		wan_modify_args,	cmdModifyWanInterface,	listCommandArgs,		"Modify interface"},
	{0,0,0,0,0,0,0,0}
};
...
#ifdef CONFIG_IGMPPROXY_MULTIWAN
static struct cmd_arg igmpprxoy_param_args[] = {
	START_SELECT_ARG
	{"ma",			STRING1,		ARG_BOTH_OPTIONAL,		ma_help},//multicast allow
	DETACH_SELECT_ARG
	{"rc",			NUMBER1,		ARG_BOTH_OPTIONAL,		rc_help},//robust count
	DETACH_SELECT_ARG
	{"lmqc",		NUMBER2,		ARG_BOTH_OPTIONAL,		lmqc_help},//last_member_query_count
	DETACH_SELECT_ARG
	{"qi",		NUMBER3,		ARG_BOTH_OPTIONAL,		qi_help},//query_interval
	DETACH_SELECT_ARG
	{"qri",		NUMBER4,		ARG_BOTH_OPTIONAL,		qri_help},//query_response_interval
	DETACH_SELECT_ARG
	{"gld",		NUMBER5,		ARG_BOTH_OPTIONAL,		gld_help},//group_leave_delay
	END_SELECT_ARG
...
#else
	{"param",		END,	CMD_SET_IGMP_PROXY_PARAM,			0,	igmpprxoy_param_args, cmdSetIgmpproxy,		listCommandArgs,	"Config igmp_proxy parameters"},
#endif
	{0,0,0,0,0,0,0,0}
};
#endif
...
#ifdef CONFIG_USER_QUAGGA
static struct cmd_node igp_rip_nodes[] = {
	{"add",		END,	CMD_ADD_IGP_RIP,		0,	igp_rip_add_args,		cmdSetIgp,	listCommandArgs,	"IGP Add RIP interface"},
	{"delete",		END,	CMD_DELETE_IGP_RIP,	0,	index_delete_args,		cmdSetIgp,	listCommandArgs,	"IGP Delete RIP interface"},
	{0,0,0,0,0,0,0,0}
};
static struct cmd_node igp_ospf_nodes[] = {
	{"add",		END,	CMD_ADD_IGP_OSPF,		0,	igp_ospf_add_args,		cmdSetIgp,	listCommandArgs,	"IGP Add OSPF interface"},
	{"delete",		END,	CMD_DELETE_IGP_OSPF,		0,	index_delete_args,			cmdSetIgp,	listCommandArgs,	"IGP Delete OSPF interface"},
	{0,0,0,0,0,0,0,0}
};
static struct cmd_node igp_bgp_network_nodes[] = {
	{"add",		END,	CMD_ADD_IGP_BGP_NETWORK,	0,	igp_bgp_network_add_args,	cmdSetIgp,	listCommandArgs,	"IGP Add BGP interface"},
	{"delete",		END,	CMD_DELETE_IGP_BGP_NETWORK,	0,	index_delete_args,			cmdSetIgp,	listCommandArgs,	"IGP Delete BGP interface"},
	{0,0,0,0,0,0,0,0}
};
static struct cmd_node igp_bgp_neighbour_nodes[] = {
	{"add",		END,	CMD_ADD_IGP_BGP_NEIGHBOUR,		0,	igp_bgp_neighbour_add_args,		cmdSetIgp,	listCommandArgs,	"IGP Add BGP interface"},
	{"delete",		END,	CMD_DELETE_IGP_BGP_NEIGHBOUR,	0,	index_delete_args,		cmdSetIgp,	listCommandArgs,	"IGP Delete BGP interface"},
	{0,0,0,0,0,0,0,0}
};
static struct cmd_node igp_isis_nodes[] = {
	{"add",		END,	CMD_ADD_IGP_ISIS,			0,	igp_isis_add_args,		cmdSetIgp,	listCommandArgs,	"IGP Add ISIS interface"},
	{"delete",		END,	CMD_DELETE_IGP_ISIS,		0,	index_delete_args,		cmdSetIgp,	listCommandArgs,	"IGP Delete ISIS interface"},
	{0,0,0,0,0,0,0,0}
};
static struct cmd_node igp_protocol_nodes[] = {
	{"rip",			0,	0,	igp_rip_nodes,			0,		0,			listNextLevelNodes,	"IGP Protocol RIP"},
	{"ospf",			0,	0,	igp_ospf_nodes,			0,		0,			listNextLevelNodes,	"IGP Protocol OSPF"},
	{"bgpnetwork",		0,	0,	igp_bgp_network_nodes,	0,		0,			listNextLevelNodes,	"IGP Protocol BGP"},
	{"bgpneighbour",	0,	0,	igp_bgp_neighbour_nodes,	0,		0,			listNextLevelNodes,	"IGP Protocol BGP"},
	{"isis",			0,	0,	igp_isis_nodes,			0,		0,			listNextLevelNodes,	"IGP Protocol ISIS"},
	{0,0,0,0,0,0,0,0}
};
static struct cmd_node cfg_igp_nodes[] = {
	{"protocol",	END,	CMD_IGP_PROTOCOL_SETTING,	0,	igp_protocol_setting_args,	cmdIgpProtocol,	listCommandArgs,"Igp Protocol Setting"},
	{"rules", 	0, 	0,					igp_protocol_nodes,	0,		0,			listNextLevelNodes,"Igp Protocol Rules"},
	{0,0,0,0,0,0,0,0}
};

static struct cmd_node show_igp_nodes[] = {
	{"protocol",	END,	CMD_SHOW_IGP_PROTOCOL,	0,	0,	cmdShowIgpProtocol,	0,		"Igp Protocol"},
	{"rip",		END,	CMD_SHOW_IGP_RIP_TBL,	0,	0,	cmdShowIgpTbl,		0,		"RIP config table"},	
	{"ospf",		END,	CMD_SHOW_IGP_OSPF_TBL,	0,	0,	cmdShowIgpTbl,		0,		"OSPF config table"},	
	{"bgp",		END,	CMD_SHOW_IGP_BGP_TBL,	0,	0,	cmdShowIgpTbl,		0,		"BGP config table"},	
	{"isis",		END,	CMD_SHOW_IGP_ISIS_TBL,	0,	0,	cmdShowIgpTbl,		0,		"ISIS config table"},	
	{0,0,0,0,0,0,0,0}
};
#endif
...
#endif
#ifdef CONFIG_USER_QUAGGA
	{"igp",			SUSERONLY, 		0,					cfg_igp_nodes,			0,					0,					listNextLevelNodes,	"Config IGP"},
#endif
#ifdef ROUTING
...
#endif
#ifdef URL_BLOCKING_SUPPORT
	{WEBURL_CHAIN,		SUSERONLY, 		0,					cfg_urlblocking_nodes,		0,					0,					listNextLevelNodes,	"Set URL blocking"},
#endif
...
#endif
#ifdef CONFIG_USER_QUAGGA
	{"igp",			0,	0,							show_igp_nodes,		0,	0,						listNextLevelNodes,	"IGP settings"},
#endif
#ifdef ROUTING
...
#endif
#ifdef URL_BLOCKING_SUPPORT
	{WEBURL_CHAIN,		END,	CMD_SHOW_URL_BLOCKING,		0,					0,	cmdShowUrlBlocking,		0,					"URL blocking"},
#endif
...
static int getWLANName(int *pvap_idx, struct cmd_arg *pCmdArg)
{
	...
	if (getArgValue(pCmdArg, "name", buf) == 1) {
		ret = sscanf(buf, VAP_IF, &wlan_idx, pvap_idx);
		switch (ret) {
		case 2:
...
/* default 
		...
		if (getArgValue(pCmdArg, "key", buf) == 1) {
			...
			if (wepKeyType == 0)
				...
				/* Hex */
				if (!rtk_string_to_hex(buf, key, keyLen)) {
	...
	if (getArgValue(pCmdArg, "key", buf) == 1) {
		...
		if (wpaPSKFormat == 1) {
			...
			if (!rtk_string_to_hex(buf, key, MAX_PSK_LEN)) {
				printf("Hex key should only contain Hex digits\n");
				goto arg_err;
			}
		} else {
...
static void cmdConfigPPP(int cmdType, struct cmd_arg *pCmdArg)
{
	int i, total;
	unsigned int index;
	unsigned char buf[MAX_PPP_NAME_LEN+1];
	...
	if (getArgValue(pCmdArg, "username", buf) == 1) {
		total++;
		strncpy(Entry.pppUsername, buf, sizeof(Entry.pppUsername));
	} else {
		printf("No username!\n");
	}
	...
	if (getArgValue(pCmdArg, "password", buf) == 1) {
		total++;
		strncpy(Entry.pppPassword, buf, sizeof(Entry.pppPassword));
	} else {
		printf("No password!\n");
	}
...
static void cmdShowPPP(int cmdType, struct cmd_arg *pCmdArg)
{
	...
	for (i = 0; i < total; i++) {
		...
		printf("%-6hhu%-15s%-*s%-*s\n", PPP_INDEX(Entry.ifIndex), ifDisplayName,
				...
				MAX_PPP_PWD_LEN, Entry.pppPassword);
	}
}
	...
	switch(cmdType){
		case CMD_ADD_RIP_INTERFACE:
			...
			if(getArgValue(pCmdArg,"sendmode",(void *)sendmode) == 1){
				...
				printf("please input rip send mode!\n");
				goto arg_err;
			}
...
static void cmdIgpProtocol(int cmdType, struct cmd_arg *pCmdArg)
{
	...
	switch(cmdType){
		case CMD_IGP_PROTOCOL_SETTING:
			if(getArgValue(pCmdArg,"protocol",(void *)protocol) == 1){
				if(!strcasecmp("NONE",protocol)){
					igpType = 0;
					mib_set(MIB_IGP_TYPE, (void *)&igpType);
				}else if(!strcasecmp("RIP",protocol)){
					igpType = 1;
					mib_set(MIB_IGP_TYPE, (void *)&igpType);
				}else if(!strcasecmp("OSPF",protocol)){
					igpType = 2;
					mib_set(MIB_IGP_TYPE, (void *)&igpType);
				}else if(!strcasecmp("BGP",protocol)){
					igpType = 3;
					mib_set(MIB_IGP_TYPE, (void *)&igpType);
				}else if(!strcasecmp("ISIS",protocol)){
					igpType = 4;
					mib_set(MIB_IGP_TYPE, (void *)&igpType);
				}
				else 
					printf("IGP Protocol must be NONE/RIP/OSPF/BGP/ISIS!");
			}else{
				printf("please input IGP Protocol!\n");
				goto arg_err;
			}
	}
...
static void cmdSetIgp(int cmdType, struct cmd_arg *pCmdArg)
{
	...
	char sendmode[64];
	char recvmode[64];
	char bgpas[5], bgpremoteas[5];
	int index,area,i;
	unsigned short as,remoteas;
	struct in_addr ipAddr, ipMask;
	unsigned char routerId[20];
	MIB_CE_RIP_T ripEntry;
	MIB_CE_OSPF_T ospfEntry;
	MIB_CE_ISIS_T isisEntry;
	MIB_CE_BGP_NEIGHBOUR_T neighEntry;
	MIB_CE_BGP_NETWORK_T networkEntry;
	char level[64];
	unsigned char isislevel,isisarea;
	
	switch(cmdType){
		case CMD_ADD_IGP_RIP:
			memset(&ripEntry,0,sizeof(MIB_CE_RIP_T));
			if(getArgValue(pCmdArg,"interface",(void *)ifname) == 1){
				if(!strcasecmp(BRIF,ifname)){
					ripEntry.ifIndex = 0xffff;
			...
			if(addRipInterface(&ripEntry) == -1){
				printf("Add rip rule error!\n");
			}
			break;
		case CMD_DELETE_IGP_RIP:
			if(getArgValue(pCmdArg,"index",(void *)&index) != 1){
				printf("Get selected rule's index number error!\n");
				goto arg_err;
			}
			if(delRipInterface(index) == -1){
				printf("Delete rip rule error!\n");
			}
			break;
		case CMD_ADD_IGP_OSPF:
			memset(&ospfEntry,0,sizeof(MIB_CE_OSPF_T));
			if(getArgValue(pCmdArg,"ipaddr",ospfEntry.ipAddr) != 1){
				printf("Get OSPF ipaddr error!\n");
				goto arg_err;
			}
			if(getArgValue(pCmdArg,"mask",ospfEntry.netMask) != 1){
				printf("Get OSPF mask error!\n");
				goto arg_err;
			}
			if(addOspfRule(&ospfEntry) == -1){
				printf("Add ospf rule error!\n");
			}
			break;
		case CMD_DELETE_IGP_OSPF:
			if(getArgValue(pCmdArg,"index",(void *)&index) != 1){
				printf("Get selected rule's index number error!\n");
				goto arg_err;
			}
			if(delOspfRule(index) == -1){
				printf("Delete ospf rule error!\n");
			}
			break;
		case CMD_ADD_IGP_BGP_NETWORK:
			memset(&networkEntry,0,sizeof(MIB_CE_BGP_NETWORK_T));
			if(getArgValue(pCmdArg,"as",(void *)bgpas) == 1){
				as=atoi(bgpas);
				if(as <1 || as > 65535)
				{
					printf("as must be 1-65535!\n");
					goto arg_err;
				}else 
					mib_set(MIB_BGP_AS, (void *)&as);
			}else {
				printf("please input bgp as!\n");
				goto arg_err;
			}
			if(getArgValue(pCmdArg,"router-id",(void *)&ipAddr) == 1){
				sprintf( routerId, "%s",   inet_ntoa(ipAddr) );
				mib_set(MIB_BGP_ROUTER_ID, (void *)routerId);
				
			}else
			{
				printf("Get bgp router-id error!\n");
				goto arg_err;
			}
			if(getArgValue(pCmdArg,"ipaddr",networkEntry.ipAddr) == 1)
			{
				if(getArgValue(pCmdArg,"mask",(void *)&ipMask) == 1)
				{
					for (i=0; i<32; i++)
						if ((ipMask.s_addr>>i) & 0x01)
							break;
					networkEntry.mask = 32 - i;
					if(addBgpNetworkRule(&networkEntry) == -1)
					{
						printf("Add network rule error!\n");
					}
				}
			}
			break;
		case CMD_DELETE_IGP_BGP_NETWORK:
			if(getArgValue(pCmdArg,"index",(void *)&index) != 1){
				printf("Get selected rule's index number error!\n");
				goto arg_err;
			}
			if(delBgpNetworkRule(index) == -1){
				printf("Delete bgp network rule error!\n");
			}
			break;
		case CMD_ADD_IGP_BGP_NEIGHBOUR:
			memset(&neighEntry,0,sizeof(MIB_CE_BGP_NEIGHBOUR_T));
			if(getArgValue(pCmdArg,"as",(void *)bgpas) == 1){
				as=atoi(bgpas);
				if(as <1 || as > 65535)
				{
			...
			if(getArgValue(pCmdArg,"ipaddr",neighEntry.ipAddr) == 1)
			{
				if(getArgValue(pCmdArg,"remote-as",(void *)bgpremoteas) == 1)
				{
					remoteas=atoi(bgpremoteas);
					if(remoteas <1 || remoteas > 65535)
					{
						printf("remote-as must be 1-65535!\n");
						goto arg_err;
					}else 
					{
						neighEntry.remoteAs=remoteas;
						if(addBgpNeighbourRule(&neighEntry) == -1)
						{
							printf("Add neighbour rule error!\n");
						}
					}
				}
			}
			break;
		case CMD_DELETE_IGP_BGP_NEIGHBOUR:
			if(getArgValue(pCmdArg,"index",(void *)&index) != 1){
				printf("Get selected rule's index number error!\n");
				goto arg_err;
			}
			if(delBgpNeighbourRule(index) == -1){
				printf("Delete bgp neighbour rule error!\n");
			}
			break;
		case CMD_ADD_IGP_ISIS:
			memset(&isisEntry,0,sizeof(MIB_CE_ISIS_T));
			if(getArgValue(pCmdArg,"area",(void *)&area) == 1){
				if(area <1 || area > 255)
				{
					printf("area must be 1-255!\n");
					goto arg_err;
				}else 
				{
					isisarea=(unsigned char)area;
					mib_set(MIB_ISIS_AREA, (void *)&isisarea);
				}
			}else {
				printf("please input isis area!\n");
				goto arg_err;
			}
			if(getArgValue(pCmdArg,"level",(void *)level) == 1){
				if(!strcasecmp("-1",level)){
					isislevel=1;
					mib_set(MIB_ISIS_TYPE, (void *)&isislevel);
				}else if(!strcasecmp("-1-2",level)){
					isislevel=2;
					mib_set(MIB_ISIS_TYPE, (void *)&isislevel);
				}else if(!strcasecmp("-2",level)){
					isislevel=3;
					mib_set(MIB_ISIS_TYPE, (void *)&isislevel);
				}else{
					printf("level must be-1/-1-2/-2!\n");
					goto arg_err;
				}	
			}else {
				printf("please input isis level!\n");
				goto arg_err;
			}
			if(getArgValue(pCmdArg,"interface",(void *)ifname) == 1){
				if(!strcasecmp(BRIF,ifname)){
					isisEntry.ifIndex= 0xffff;
				}else{
					isisEntry.ifIndex = getIfIndexByName(ifname);
				}
			}else{
				printf("please input interface name!\n");
				goto arg_err;
			}
			if(addIsisRule(&isisEntry) == -1){
				printf("Add isis rule error!\n");
			}
			break;
		case CMD_DELETE_IGP_ISIS:
			if(getArgValue(pCmdArg,"index",(void *)&index) != 1){
				printf("Get selected rule's index number error!\n");
				goto arg_err;
			}
			if(delIsisRule(index) == -1){
				printf("Delete ospf rule error!\n");
			}
			break;
	}
...
{
	switch(cmdType){
		case CMD_SHOW_IGP_RIP_TBL:
			showIgpRip();
			break;
		case CMD_SHOW_IGP_OSPF_TBL:
			showIgpOspf();
			break;
		case CMD_SHOW_IGP_BGP_TBL:
			showIgpBgp();
			break;
		case CMD_SHOW_IGP_ISIS_TBL:
			showIgpIsis();
			break;
	}
}
...
static void cmdSetIgmpproxy(int cmdType, struct cmd_arg *pCmdArg)
{
	...
	switch(cmdType){
		...
		case CMD_ENABLE_IGMP_PROXY:
			...
			break;
#else
		case CMD_SET_IGMP_PROXY_PARAM:
			if(getArgValue(pCmdArg, "ma", (void*)ma) == 1)
			{
				if (!strcasecmp("disable",ma))
					vInt = 0;
				else if (!strcasecmp("enable",ma))
					vInt = 1;
				else
				{
					printf("ma param error!\n");
						return;
				}
					flag = 1;
				mib_set(MIB_IGMP_MULTICAST_ALLOW, (void *)&vInt);
			}
			if(getArgValue(pCmdArg, "rc", (void*)&vInt) == 1)
			{
				flag = 1;
				mib_set(MIB_IGMP_ROBUST_COUNT, (void *)&vInt);
			}
			if(getArgValue(pCmdArg, "lmqc", (void*)&vInt) == 1)
			{
				flag = 1;
				mib_set(MIB_IGMP_LAST_MEMBER_QUERY_COUNT, (void *)&vInt);
			}
			if(getArgValue(pCmdArg, "qi", (void*)&vInt) == 1)
			{
				flag = 1;
				mib_set(MIB_IGMP_QUERY_INTERVAL, (void *)&vInt);
			}
			if(getArgValue(pCmdArg, "qri", (void*)&vInt) == 1)
			{
				flag = 1;
				mib_set(MIB_IGMP_QUERY_RESPONSE_INTERVAL, (void *)&vInt);
			}
			if(getArgValue(pCmdArg, "gld", (void*)&vInt) == 1)
			{
				flag = 1;
...
static void cmdAddWanInterface(int cmdType, struct cmd_arg *pCmdArg)
{
...
#ifdef CONFIG_IGMPPROXY_MULTIWAN
	/*igmpproxy*/
	if (getArgValue(pCmdArg, "igmpproxy", buf) == 1) {
		if ( !strcmp(buf, "enable"))
			entry.enableIGMP = 1;
		else if(!strcmp(buf, "disable"))
			entry.enableIGMP = 0;
		else
			printf("Please input igmpproxy with enable or disable!\n");
	}
#endif
	...
	//IpProtocol
	if (getArgValue(pCmdArg, "ipProtocol", &intVal) == 1) {
		entry.IpProtocol=intVal;
	}
...
#endif
	{
		...
		/* Request DNS */
		entry.dnsMode = 1;
		if (getArgValue(pCmdArg, "ipv4Dns1", buf) == 1) {
			if (!inet_aton(buf, (struct in_addr *)&entry.v4dns1)) {
				printf("Invalid ipv4Dns1 address!\n");
				goto arg_err;
			}
			entry.dnsMode = 0;
		}

		if (getArgValue(pCmdArg, "ipv4Dns2", buf) == 1) {
			if (!inet_aton(buf, (struct in_addr *)&entry.v4dns2)) {
				printf("Invalid ipv4Dns2 address!\n");
				goto arg_err;
			}
			entry.dnsMode = 0;
		}
	...
	{
		if(getArgValue(pCmdArg, "ipv6Addr", buf) == 1)
		{
			if (!inet_pton(PF_INET6, buf,&ip6Addr) )
			{
				printf("Invalid  ipv6Addr!\n");
				goto arg_err;
			}
			memcpy(entry.Ipv6Addr, &ip6Addr, sizeof(entry.Ipv6Addr));

			if (getArgValue(pCmdArg, "ipv6PrefixLen", &intVal) == 1)
			{
				entry.Ipv6AddrPrefixLen=intVal;
			}else
			{
				printf("Please input ipv6PrefixLen!\n");
				goto arg_err;
			}

			if (getArgValue(pCmdArg, "ipv6Gateway", buf) == 1)
			{
				if(!inet_pton(PF_INET6, buf, &ip6Addr))
				{
					printf("Invalid ipv6Gateway!\n");
					goto arg_err;
				}
				memcpy(entry.RemoteIpv6Addr, &ip6Addr, sizeof(entry.RemoteIpv6Addr));
			}
			else
			{
				printf("Please input ipv6Gateway!\n");
				goto arg_err;
			}
		...
		else if(getArgValue(pCmdArg, "slaac",buf) == 1)
		{
			entry.AddrMode=IPV6_WAN_AUTO;
			entry.Ipv6Dhcp = 1;
		}

		entry.Ipv6DhcpRequest =0;
		if (getArgValue(pCmdArg, "iana", buf) == 1)
		{
			if (!strcmp("on", buf) || !strcmp("ON", buf)) {
				entry.Ipv6DhcpRequest |= M_DHCPv6_REQ_IANA;
			}
		}

		if (getArgValue(pCmdArg, "iapd", buf) == 1)
		{
			if (!strcmp("on", buf) || !strcmp("ON", buf)) {
				entry.Ipv6DhcpRequest |= M_DHCPv6_REQ_IAPD;
			}
		}

		entry.dnsv6Mode = 1;
		if (getArgValue(pCmdArg, "ipv6Dns1", buf) == 1)
		{
			if (!inet_pton(PF_INET6, buf,&ip6Addr))
			{
				printf("Invalid ipv6Dns1 address!\n");
				goto arg_err;
			}
			memcpy(entry.Ipv6Dns1, &ip6Addr, sizeof(entry.Ipv6Dns1));
			entry.dnsv6Mode = 0;
		}
		if (getArgValue(pCmdArg, "ipv6Dns2", buf) == 1) {
			if (!inet_pton(PF_INET6, buf,&ip6Addr)) {
				printf("Invalid ipv6Dns2 address!\n");
				goto arg_err;
			}
			memcpy(entry.Ipv6Dns2, &ip6Addr, sizeof(entry.Ipv6Dns2));
			entry.dnsv6Mode = 0;
		}
	}
#endif
...
static void cmdModifyWanInterface(int cmdType, struct cmd_arg *pCmdArg)
{
	...
	for (i=0; i<totalNum; i++)
	{
		...
		getDisplayWanName(&Entry, ifDisplayName);
		if (!strcmp(ifname, ifDisplayName))
			break;
	}
	if (i >= totalNum)
	{
	...
	if(check_vlan_conflict(&entry, i, tmpBuf) != 0)
	{
		printf("%s\n", tmpBuf);
		goto arg_err;
	}
#ifdef CONFIG_IGMPPROXY_MULTIWAN
	/*igmpproxy*/
	if (getArgValue(pCmdArg, "igmpproxy", tmpBuf) == 1) {
		if ( !strcmp(tmpBuf, "enable"))
			entry.enableIGMP = 1;
		else if(!strcmp(tmpBuf, "disable"))
			entry.enableIGMP = 0;
		else
			printf("Please input igmpproxy with enable or disable!\n");
	}
...
extern int cmdedit_read_input(char *prompt, char command[BUFSIZ]);

int main(int argc, char *argv[])
{
	int auth_denied=1; //auth_denied=1: Login Incorrect, auth_denied=0: Login Success
	int flag, len, login=0, user_flag = 0;
