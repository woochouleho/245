...
int sent, brd_sent;
int received, received_last = 0, brd_recv, req_recv;
...
static void log_pid()
{
		...
		sprintf(pidfile, "%s_%s", ARPING_IPOE_RUNFILE, device);
...
{
	fprintf(stderr,
 		"Usage: arping [-fqbDUAVd] [-c count] [-w timeout] [-t interval] [-I device] [-s source] destination\n"
		...
		"  -t interval : unicast arp interval in seconds\n"
...
void catcher(void)
{
	...
	if (last.tv_sec==0 || MS_TDIFF(tv,last) > 500) {
		if (retry > 0)
		{
			int diff = (sent - received_last);
			if (debug == 1)
				printf("retry = %d, sent = %d, received_last = %d \n", retry, sent, received_last);

			if (diff > 0)
			{
				if (debug == 1)
					printf("Unicast lost!!\n");
				sprintf(cmd_str, "/bin/echo \"Unicast lost 0\" >> %s_%s", ARPING_RESULT, device);
				system(cmd_str);
...
int recv_pack(unsigned char *buf, int len, struct sockaddr_ll *FROM)
{
	...
	if (!quiet) {
		...
		sprintf(cmd_str, "/bin/echo \"Unicast reply 1\" >> %s_%s", ARPING_RESULT, device);
		system(cmd_str);
	...
	received++;
	received_last = sent;
	if(received < 0 || received == INT_MAX)
		received = 0;
	if (FROM->sll_pkttype != PACKET_HOST)
		brd_recv++;
	if (ah->ar_op == htons(ARPOP_REQUEST))
...
main(int argc, char **argv)
{
	...
	while ((ch = getopt(argc, argv, "h?bdfDUAqc:w:t:s:r:I:V")) != EOF) {
		switch(ch) {
		case 'b':
			broadcast_only=1;
			break;
		...
		case 'r':
			...
			break;
		case 'V':
			printf("arping utility, iputils/arping_ipoe\n");
	...
	set_signal(SIGTERM, quit_signal);
	set_signal(SIGQUIT, quit_signal);
	catcher();
