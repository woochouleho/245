...
char *getMibInfo(int id) {
	if (getMIB2Str_s(id, strbuf, sizeof(strbuf)) == 0)
		return strbuf;
...
int getTypedInputDefault(int type, char *prompt, void *rv, void *arg1, void *arg2) {
	while (1) {
		...
		if (INPUT_TYPE_STRING == type) {
			char *pc;
			long vLong = (long)arg1;
			fgets((char *)rv, (int)vLong, stdin);
			pc = (char *)rv;
...
int getInputString(char *prompt, char *rv, int len) {
	long len2 = len;
	return (getTypedInput(INPUT_TYPE_STRING, prompt, (void *)rv, (void *)len2, 0));
}
...
int setAccountPassword(char * user, char *oldpasswd, char *newpasswd)
{
	...
	char oldMIBpass[MAX_PASSWD_LEN];
...
check_oldpwd:
	...
	if (selnum == 1) {
		...
		strncpy(Entry.userPassword, newpasswd, MAX_PASSWD_LEN-1);
...
int addDhcpReservedIp(struct in_addr *ip,char *pMac6)
{
	...
	mib_get_s(MIB_DHCP_MODE, (void *)&mode, sizeof(mode));
	if(mode != DHCPV4_LAN_SERVER){
...
void showDmzSetting()
{
	...
	mib_get_s(MIB_DMZ_ENABLE, (void *)&vChar, sizeof(vChar));
	if(vChar == 1){
		printf("DMZ is enabled\n");
		mib_get_s(MIB_DMZ_IP, (void *)&ipAddr, sizeof(ipAddr));
		printf("DMZ host: %s\n",inet_ntoa(ipAddr));
	}else{
...
void showDnsSetting()
{
	...
	mib_get_s(MIB_ADSL_WAN_DNS_MODE, (void *)&vChar, sizeof(vChar));
...
void showDomainBlocking()
{
	...
	mib_get_s(MIB_DOMAINBLK_CAPABILITY, (void *)&vChar, sizeof(vChar));
...
#ifdef CONFIG_TRUE
		if (CHANNEL_MODE_PPPOE == Entry.cmode)
		{
...
{
	unsigned char proxy;
	unsigned int ifIndex, entryNum, i;
	...
	for (i=0; i<entryNum; i++) {
		...
		printf("Query Interval:          %d(seconds)\n",vInt);
		mib_get(MIB_IGMP_QUERY_RESPONSE_INTERVAL, (void *)&vInt);
		printf("Query Response Interval: %d(*100ms)\n",vInt);
		mib_get(MIB_IGMP_GROUTP_LEAVE_DELAY, (void *)&vInt);
		printf("Group Leave Delay:       %d(ms)\n",vInt);
	}
#else
...
void showUrlBlocking()
{
	...
	mib_get_s(MIB_URL_CAPABILITY, (void *)&vChar, sizeof(vChar));
...
void showWLAN(void)
{
	...
	mib_get_s(MIB_WLAN_DISABLED, &wlanDisabled, sizeof(wlanDisabled));
	mib_get_s(MIB_WLAN_ENCRYPT, &encrypt, sizeof(encrypt));
	mib_get_s(MIB_WLAN_SSID, ssid, sizeof(ssid));
...
int setUpnpCapability(unsigned char enabled,unsigned int ifIndex)
{
	unsigned char pre_enabled;
	unsigned int  pre_ext_if;
	char ifname[IFNAMSIZ];

	mib_get(MIB_UPNP_DAEMON, (void *)&pre_enabled);
	mib_get(MIB_UPNP_EXT_ITF, (void *)&pre_ext_if);
	...
	if(enabled){
		...
		//printf("[%s %d]pre_ext_if=%d\n",__func__,__LINE__,pre_ext_if);
		if(getIfModeByIndex(ifIndex) <= CHANNEL_MODE_BRIDGE){
			printf("no such route interface!\n");
			return -1;
		}

		if((pre_enabled && (ifIndex != pre_ext_if))) 
		{
...
void showPortForwarding()
{
	...
	mib_get_s( MIB_PORT_FW_ENABLE, (void *)&enabled, sizeof(enabled));
...
void showRipStatusTbl()
{
	...
	mib_get_s(MIB_RIP_ENABLE, (void *)&enabled, sizeof(enabled));
	if(enabled == 0){
...
int addRipInterface(MIB_CE_RIP_Tp pEntry)
{
	int entryNum, i, intVal;
	MIB_CE_RIP_T ripEntry;
	unsigned char oldIgpType;
	mib_get(MIB_IGP_TYPE, (void *)&oldIgpType);
...
int addOspfRule(MIB_CE_OSPF_Tp pEntry)
{
	int entryNum, i, intVal;
	...
	for (i=0; i<entryNum; i++) {
		mib_chain_get(MIB_OSPF_TBL, i, (void *)&ospfEntry);
		if ((strcmp(ospfEntry.ipAddr,pEntry->ipAddr) == 0)&&(strcmp(ospfEntry.netMask,pEntry->netMask) == 0)) {
			printf("Entry already exists!\n");
			return -1;
		}
	}

	intVal = mib_chain_add(MIB_OSPF_TBL, (unsigned char*)pEntry);
	if (intVal == 0) {
		printf(strAddChainerror);
		return -1;
	}else if (intVal == -1) {
...
int delOspfRule(int index)
{
	unsigned int totalEntry;
	unsigned char oldIgpType;
	mib_get(MIB_IGP_TYPE, (void *)&oldIgpType);

	totalEntry = mib_chain_total(MIB_OSPF_TBL); /* get chain record size */
	if(totalEntry == 0){
		printf("Empty table!\n");
		return -1;
	}
	...
	if(mib_chain_delete(MIB_OSPF_TBL, index - 1) != 1)
	{
		printf("Delete chain record error!\n");
		return -1;
	}
...
int addIsisRule(MIB_CE_ISIS_Tp pEntry)
{
	int entryNum, i, intVal;
	...
	for (i=0; i<entryNum; i++) {
		mib_chain_get(MIB_ISIS_TBL, i, (void *)&isisEntry);
		if (isisEntry.ifIndex == pEntry->ifIndex) {
			printf("Entry already exists!\n");
			return -1;
		}
	}
	intVal = mib_chain_add(MIB_ISIS_TBL, (unsigned char*)pEntry);
	if (intVal == 0) {
		printf(strAddChainerror);
		return -1;
	}else if (intVal == -1) {
...
int delIsisRule(int index)
{
	unsigned int totalEntry;
	unsigned char oldIgpType;
	mib_get(MIB_IGP_TYPE, (void *)&oldIgpType);

	totalEntry = mib_chain_total(MIB_ISIS_TBL); /* get chain record size */
	if(totalEntry == 0){
		printf("Empty table!\n");
		return -1;
	}
	...
	if(mib_chain_delete(MIB_ISIS_TBL, index - 1) != 1)
	{
		printf("Delete chain record error!\n");
		return -1;
	}
...
int addBgpNetworkRule(MIB_CE_BGP_NETWORK_Tp pEntry)
{
	int entryNum, i, intVal;
	...
	for (i=0; i<entryNum; i++) {
		mib_chain_get(MIB_BGP_NETWORK_TBL, i, (void *)&networkEntry);
		if ((strcmp(networkEntry.ipAddr,pEntry->ipAddr) == 0)&&(networkEntry.mask==pEntry->mask)) {
			printf("Entry already exists!\n");
			return -1;
		}
	}

	intVal = mib_chain_add(MIB_BGP_NETWORK_TBL, (unsigned char*)pEntry);
	if (intVal == 0) {
		printf(strAddChainerror);
		return -1;
	}else if (intVal == -1) {
...
int delBgpNetworkRule(int index)
{
	unsigned int totalEntry;
	unsigned char oldIgpType;
	mib_get(MIB_IGP_TYPE, (void *)&oldIgpType);

	totalEntry = mib_chain_total(MIB_BGP_NETWORK_TBL); /* get chain record size */
	if(totalEntry == 0){
		printf("Empty table!\n");
		return -1;
	}
	...
	if(mib_chain_delete(MIB_BGP_NETWORK_TBL, index - 1) != 1)
	{
		printf("Delete chain record error!\n");
		return -1;
	}
...
int addBgpNeighbourRule(MIB_CE_BGP_NEIGHBOUR_Tp pEntry)
{
	int entryNum, i, intVal;
	...
	for (i=0; i<entryNum; i++) {
		mib_chain_get(MIB_BGP_NEIGHBOUR_TBL, i, (void *)&neighbourEntry);
		if ((strcmp(neighbourEntry.ipAddr,pEntry->ipAddr) == 0)&&(neighbourEntry.remoteAs == pEntry->remoteAs)) {
			printf("Entry already exists!\n");
			return -1;
		}
	}

	intVal = mib_chain_add(MIB_BGP_NEIGHBOUR_TBL, (unsigned char*)pEntry);
	if (intVal == 0) {
		printf(strAddChainerror);
		return -1;
	}else if (intVal == -1) {
...
int delBgpNeighbourRule(int index)
{
	unsigned int totalEntry;
	unsigned char oldIgpType;
	mib_get(MIB_IGP_TYPE, (void *)&oldIgpType);

	totalEntry = mib_chain_total(MIB_BGP_NEIGHBOUR_TBL); /* get chain record size */
	if(totalEntry == 0){
		printf("Empty table!\n");
		return -1;
	}
	...
	if(mib_chain_delete(MIB_BGP_NEIGHBOUR_TBL, index - 1) != 1)
	{
		printf("Delete chain record error!\n");
		return -1;
	}
...
void showIgpProtocol()
{
	unsigned char igpType;

	mib_get(MIB_IGP_TYPE, (void *)&igpType);
	if(igpType == 0)
	{
		printf("IGP Protocol : NONE\n");
	}
	else if(igpType == 1)
	{
		printf("IGP Protocol : RIP\n");
	}
	else if(igpType == 2)
	{
		printf("IGP Protocol : OSPF\n");
	}
	else if(igpType == 3)
	{
		printf("IGP Protocol : BGP\n");
	}
	else if(igpType == 4)
	{
		printf("IGP Protocol : ISIS\n");
	}
}
void showIgpRip()
{
...
void showIgpOspf()
{
	...
	MIB_CE_OSPF_T Entry;
	char net[20];
	unsigned int uMask,uIp;

	entryNum = mib_chain_total(MIB_OSPF_TBL);
	printf("The following is OSPF interface table:\n");
	printf("Index\tOSPF Network\n");
	MSG_LINE;
	if (!entryNum) {
		printf("No data!\n\n");
		return;
	...
	for (i=0; i<entryNum; i++) {
	mib_chain_get(MIB_OSPF_TBL, i, (void *)&Entry);
...
void showIgpBgp()
{
	...
	mib_get(MIB_BGP_ROUTER_ID, (void *)routerId);
	printf("AS:%d\trouter-id:%s\n",bgpAs,routerId);
	
	entryNum = mib_chain_total(MIB_BGP_NEIGHBOUR_TBL);
	printf("The following is BGP Neighbour interface table:\n");
	printf("Index\tBGP Neighbour\tremote-as\n");
	MSG_LINE;
	if (!entryNum) {
		printf("No data!\n\n");
	}
	...
	{
		for (i=0; i<entryNum; i++) {
		mib_chain_get(MIB_BGP_NEIGHBOUR_TBL, i, (void *)&neighEntry);
		uIp = *(unsigned int *)neighEntry.ipAddr;
		remoteAS = neighEntry.remoteAs;
		sprintf(net, "%s", inet_ntoa(*((struct in_addr *)&uIp)));
	...
	MENU_LINE;
	entryNum = mib_chain_total(MIB_BGP_NETWORK_TBL);
	printf("The following is BGP Network interface table:\n");
	printf("Index\tBGP Network\n");
	MSG_LINE;
	if (!entryNum) {
		printf("No data!\n\n");
		return;
...
void showIgpIsis()
{
	...
	mib_get(MIB_ISIS_AREA, (void *)&area);
	mib_get(MIB_ISIS_TYPE, (void *)&level);
	if(level == 1)
		isislevel=level_1;
	else if(level == 2)
		isislevel=level_1_2;
		...
		isislevel=level_2;
	printf("area:%d\trouter-id:%s\n",area,isislevel);

	entryNum = mib_chain_total(MIB_ISIS_TBL);
	printf("The following is ISIS interface table:\n");
	printf("Index\tISIS Iface\n");
	MSG_LINE;
	if (!entryNum) {
		printf("No data!\n\n");
		return;
	...
	for (i=0; i<entryNum; i++) {
		...
		if( isisEntry.ifIndex == DUMMY_IFINDEX) {
			strncpy(ifname, BRIF, strlen(BRIF));
			ifname[strlen(BRIF)] = '\0';
		} else {
			ifGetName(isisEntry.ifIndex, ifname, sizeof(ifname));
		}
		printf("%d\t%s\n", i+1, ifname);
	}
}
...
int staticRouteAdd(MIB_CE_IP_ROUTE_Tp pEntry)
{
	...
	ret=route_cfg_modify(pEntry, 0, totalEntry-1);
...
int isSnmpEnabled()
{
	...
	mib_get_s(MIB_SNMPD_ENABLE, (void *)&enabled, sizeof(enabled));
	if(enabled == 0)
		return 0;
	else
		return 1;
...
int setDSLSettings(unsigned short mode, char olr, unsigned short profile)
{
	...
	//mib_get_s(MIB_ADSL_MODE, (void *)&oldMode, sizeof(oldMode));
	mib_get_s( MIB_DSL_ANNEX_B, (void *)&dsl_anxb, sizeof(dsl_anxb));
...
//		pEntry->dgw = 1;
	...
	/* it seems LINK change signal always be triggered before dhcpc starting, so we should re-kill SIGUSR1 to dhcpc again */
	if ((CHANNEL_MODE_IPOE == pEntry->cmode) && ((DHCP_T)pEntry->ipDhcp != DHCP_DISABLED))
	{
		char sVal[32], ifname[IFNAMSIZ];
		int dhcpc_pid;
		...
		if(dhcpc_pid > 0) kill(dhcpc_pid, SIGUSR1); // force renew
	}
...
#ifdef ITF_GROUP
	{
		...
		for (grpnum=1; grpnum<=4; grpnum++) {
			...
			if (0 == wanPortNum) {
				...
				// release wlan1
				mib_get_s(MIB_WLAN1_ITF_GROUP, (void *)&mygroup, sizeof(mygroup));
				if (mygroup == grpnum) {
					mygroup = 0;
				...
				for (j=1; j<IFGROUP_NUM; j++) {
					mib_get_s(MIB_WLAN_VAP0_ITF_GROUP + ((j-1)<<1), (void *)&mygroup, sizeof(mygroup));
					if (mygroup == grpnum) {
						mygroup = 0;
				...
				for (j=1; j<IFGROUP_NUM; j++) {
					mib_get_s(MIB_WLAN1_VAP0_ITF_GROUP + ((j-1)<<1), (void *)&mygroup, sizeof(mygroup));
					if (mygroup == grpnum) {
						mygroup = 0;
