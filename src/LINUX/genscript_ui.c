...
#define _DHCPD_LEASE_FILE_				("/var/udhcpd/udhcpd.leases")
	...
	if( strcmp(enable, "off") != 0) {		
		...
		mib_get_s(MIB_ADSL_LAN_DHCP, (void *)&vChar, sizeof(vChar)); /* get LAN DHCP MODE*/
		...
		if ((out = fdopen(fh, "w")) != NULL) {
			...
			mib_get_s(MIB_ADSL_LAN_CLIENT_START, (void *)&ip_addr, sizeof(ip_addr));
			fprintf(out, "start %s\n", inet_ntoa(ip_addr));

			mib_get_s(MIB_ADSL_LAN_CLIENT_END, (void *)&ip_addr, sizeof(ip_addr));
			fprintf(out, "end %s\n", inet_ntoa(ip_addr));

			mib_get_s(MIB_ADSL_LAN_SUBNET, (void *)&ip_addr, sizeof(ip_addr));
			fprintf(out, "opt subnet %s\n", inet_ntoa(ip_addr));

			mib_get_s(MIB_ADSL_LAN_IP, (void *)&ip_addr, sizeof(ip_addr));
			...
			if(vChar == 0) {	/* Auto DNS Mode, DNS relay  */
				...
				mib_get_s(MIB_ADSL_WAN_DNS1, (void *)&ip_addr, sizeof(ip_addr));
				fprintf(out, "opt dns %s\n", inet_ntoa(ip_addr));
				mib_get_s(MIB_ADSL_WAN_DNS2, (void *)&ip_addr, sizeof(ip_addr));
				fprintf(out, "opt dns %s\n", inet_ntoa(ip_addr));
				mib_get_s(MIB_ADSL_WAN_DNS3, (void *)&ip_addr, sizeof(ip_addr));
				fprintf(out, "opt dns %s\n", inet_ntoa(ip_addr));
			}
...
int genscript_initlan(int argc, char** argv, int argNum)
{
	...
	if ((out = fdopen(fh, "w")) != NULL) {
		if( !strcmp(enable, "off") ) {
			...
			mib_get_s(MIB_ADSL_LAN_IP, (void *)&ip_addr, sizeof(ip_addr));
			mib_get_s(MIB_ADSL_LAN_SUBNET, (void *)&subnet, sizeof(subnet));
			mib_get_s(MIB_ADSL_LAN_GATEWAY, (void *)&gw, sizeof(gw));
			mib_get_s(MIB_ADSL_LAN_DHCP, (void *)&vChar, sizeof(vChar));
...
#define _IP_DYNADDR_FILE_			("/proc/sys/net/ipv4/ip_dynaddr")
...

	{
		...
		mib_get_s(MIB_PORT_FW_ENABLE, (void *)&vChar, sizeof(vChar));
		...
		mib_get_s(MIB_IP_PORT_FILTER_MODE, (void *)&vChar, sizeof(vChar));
		entryNum = mib_chain_total(MIB_IP_PORT_FILTER_TBL);
		printf("IP PORT FILTER %d %d\n", vChar, entryNum);
#endif

#ifdef MAC_FILTER
		mib_get_s(MIB_MAC_FILTER_MODE, (void *)&vChar, sizeof(vChar));
	...
	if ((out = fdopen(fh, "w")) != NULL) {
		...
		if( !strcmp(enable, "on") ) {
			...
			if(vChar == 1) {
				...
				memset(dmz, 0x00, 16);
				mib_get_s(MIB_DMZ_IP, (void *)&ip_addr, sizeof(ip_addr));
