...
#ifdef CONFIG_VDSL
#define ACCESS_PROTOCOL "VDSL"
#elif defined(CONFIG_GPON_FEATURE) || defined(CONFIG_EPON_FEATURE) || defined(CONFIG_FIBER_FEATURE)
#define ACCESS_PROTOCOL "PON"
#else
#define ACCESS_PROTOCOL "ADSL"
#endif /*CONFIG_VDSL*/
...
void showClient() // Jenny
{
	...
	for (i=1; i<=MAX_STA_NUM; i++) {
		...
		if (pInfo->aid && (pInfo->flags & STA_INFO_FLAG_ASOC)) {
			...
			printf("%-19s%-10lu%-10lu%-8s%-13s%lu\n",
#elif defined(WLAN_11AX)
			printf("%-19s%-10llu%-10llu%-8s%-13s%u\n",
#else
			printf("%-19s%-10lu%-10lu%-8s%-13s%lu\n",
#endif
				tmpbuf, pInfo->tx_packets, pInfo->rx_packets, txrate,
				( (pInfo->flags & STA_INFO_FLAG_ASLEEP) ? "Yes" : "No"), pInfo->expired_time/100);
		}
	}
	free(buff);
	printWaitStr();
...
#endif
		if (wl_isNband(band)) {
			mib_get_s(MIB_WLAN_CHANNEL_WIDTH, (void *)strbuf, sizeof(strbuf));
			printf("Channel Width: %s\n", strbuf[0]? "40MHz": "20MHz");
			mib_get_s(MIB_WLAN_CONTROL_BAND, (void *)strbuf, sizeof(strbuf));
			printf("Control Sideband: %s\n", strbuf[0]? "Lower": "Upper");
		}
		mib_get_s( MIB_WLAN_AUTO_CHAN_ENABLED, (void *)&auto_chan, sizeof(auto_chan));
		if(auto_chan)
...
//Magician: Commit immediately
		...
		case 1: //(1) Set
			...
			//mib_get_s(MIB_WLAN_DISABLED, (void *)&wlan, sizeof(wlan)); // Jenny, get current wireless interface status
			if (wlan == 0) { // enable
...
void setWlanAdv()
{
	...
	while (1)
	{
		...
		mib_get_s(MIB_WLAN_PROTECTION_DISABLED, (void *)&protection, sizeof(protection));  // Magician
		printf("Protection: %s\n", protection? "Disabled": "Enabled");
		mib_get_s(MIB_WLAN_AGGREGATION, (void *)&aggre, sizeof(aggre));
		printf("Aggregation: %s\n", (aggre & (1<<WLAN_AGGREGATION_AMPDU))? "Enabled": "Disabled");
		mib_get_s(MIB_WLAN_SHORTGI_ENABLED, (void *)&shgi, sizeof(shgi));
		...
		case 1://(1) Set
			...
			min=MIN_WLAN_BEACON_INTERVAL;max=1024;
			snprintf(strbuf, sizeof(strbuf), "Beacon Interval (%u ~ 1024 ms): ", min);
			if (0 == getInputUint(strbuf, &num, &min, &max))
				continue;
			...
			#ifdef WPS20
			mib_get_s(MIB_WSC_VERSION, (void *) &wscUseVersion, sizeof(wscUseVersion));
				...
				if(enableRS == 0)
				{
					...
					if (strlen(tmpBuf) == len) {
						if (keyfmt == 0) // ASCII
							...
							if (!rtk_string_to_hex(tmpBuf, key1, len))
								continue;
						}
					}
				...
				if (getPSK == 1) {
					...
					if (pskfmt == 0) {// Passphrase
						...
						if (0 == getInputString("Pre-Shared Key (64 chars): ", psk, 66))
							continue;
						len = strlen(psk);
						if (len != 64 || !rtk_string_to_hex(psk, tmpBuf, 64))
							continue;
					}
...
int addWLanAC(unsigned char *mac) // Jenny
{
	...
	if (strlen(mac) != 12 || !rtk_string_to_hex(mac, macEntry.macAddr, 12) || !isValidMacAddr(macEntry.macAddr)) {
		...
		switch(vChar)
		{
			...
			case WSC_AUTH_WPA2PSK:
				...
				if(Entry.encrypt == WIFI_SEC_WPA3)
					strcpy(auth, "WPA3 SAE");
				else if(Entry.encrypt == WIFI_SEC_WPA2_WPA3_MIXED)
#ifdef WLAN_WPA3_H2E
					strcpy(auth, "WPA3 Transition");
#else
					strcpy(auth, "WPA2+WPA3-Mixed PSK");
#endif
				else
					strcpy(auth, "WPA2 PSK");
#else
				strcpy(auth, "WPA2 PSK");
#endif
				break;
...
void setWlWPS()  // Magician
{
	...
	while (1)
	{
		...
		mib_get_s(MIB_WSC_PIN, (void *)strbuf, sizeof(strbuf));
...
int delWLanWDS()
{
	...
	delNum=0;
	mib_get_s(MIB_WLAN_WDS_NUM, (void *)&entryNum, sizeof(entryNum)); /* get chain record size */
	if (entryNum==0){
		printf("Empty table!\n");
		return 0;
	}
...
void showWLanWDS()
{
	...
	mib_get_s(MIB_WLAN_WDS_NUM, (void *)&entryNum, sizeof(entryNum));
	for (i=0; i<entryNum; i++) {
		*((char *)&entry) = (char)i;
...
void showWlSiteSurvey()
{
	...
	if ( getWlSiteSurveyResult((char *)WLANIF[0], pStatus) < 0 ) {
...
void connectWlSiteSurvey()
{
	...
	wait_time = 0;
	while (1) {
		if ( getWlJoinRequest((char*)WLANIF[0], &pStatus->bssdb[idx], &res) < 0 ) {
	...
	if ( res == 2 ) // invalid index
		...
		wait_time = 0;
		while (1) {
			if ( getWlJoinResult((char*)WLANIF[0], &res) < 0 ) {
...
void showEthWanConfiguration(MEDIA_TYPE_T m_type)
{
	...
	const char *mode;
	char *strNapt, ifname[IFNAMSIZ], ipAddr[20], ipMask[20], remoteIp[20], UserName[MAX_PPP_NAME_LEN+1], PassWord[MAX_PPP_PWD_LEN+1];
	char *temp;
	...
	for(i=0;i<mibtotal;i++){
		...
		if(MEDIA_INDEX(Entry.ifIndex) == m_type){
			...
			if (Entry.cmode == CHANNEL_MODE_PPPOE)
			{
				strncpy(UserName, Entry.pppUsername, sizeof(UserName));
				UserName[MAX_PPP_NAME_LEN] = '\0';
				strncpy(PassWord, Entry.pppPassword, sizeof(PassWord));
				PassWord[MAX_PPP_PWD_LEN] = '\0';
...
static int getEthWanInput(MIB_CE_ATM_VC_Tp entry, MIB_CE_ATM_VC_Tp old)
{
	...
	if(entry->vlan == 1){
		min = 0; max = 4095;
		snprintf(tmpbuf, 64, "VLAN ID(0, %d~%d are reserved) :", RESERVED_VID_START, RESERVED_VID_END);
		if(0 == getInputUint(tmpbuf, &num, &min, &max))
			return 0;
		if (check_vlan_reserved(num))
			return 0;
		entry->vid = num;
		min = 0; max = 7;
	...
	entry->enableIpQos = num-1;
		...
		if (entry->cmode == CHANNEL_MODE_PPPOE)
		{
			...
			if (0 == getInputString("Password: ", entry->pppPassword, MAX_PPP_PWD_LEN))
				return 0;
			min=1;max=3;
...
void setAutoPVCSearch()
{
	...
	while (1)
	{
		...
		MENU_LINE;
		mib_get_s(MIB_ATM_VC_AUTOSEARCH, (void *)&enabled, sizeof(enabled));
...
void changeADSLSettings(XDSL_OP *d){
	...
	mib_get_s(MIB_ADSL_MODE, (void *)&mode, sizeof(mode));
	mib_get_s( MIB_DSL_ANNEX_B, (void *)&anxb, sizeof(anxb));
	//if (mode & ADSL_MODE_ANXB) {
	if (anxb) {
		...
		switch( snum)
		{
			case 1://(1) Set DHCP Mode
				...
				if (!getInputOption( &snum, 0, 2))
					break;
				if ( snum==0 )
					dtmode = DHCPV4_LAN_NONE;
				else if ( snum == 1 )
					dtmode = DHCPV4_LAN_RELAY;
				else if ( snum == 2 )
					dtmode = DHCPV4_LAN_SERVER;
				else {
					printf("Invalid selection\n");
					break;
				}
...
void addMacBase()
{
	...
	mac[12] = '\0';
	if (strlen(mac) != 12  || !rtk_string_to_hex(mac, entry.macAddr_Dhcp, 12) || !isValidMacAddr(entry.macAddr_Dhcp)) {
		printf("Error! Invalid MAC address.");
		goto setErr_mac;
	}
...
/*ping_zhang:20090930 START:add for Telefonica new option 240*/
		...
		if(Entry.deviceType == CTC_STB) {
			mib_get_s(MIB_OPCH_ADDRESS, (void *)&opchAddr, sizeof(opchAddr));
			mib_get_s(MIB_OPCH_PORT, (void *)&opchport, sizeof(opchport));
...
// Kaohj
#ifdef CONFIG_SECONDARY_IP
	mib_get_s( MIB_ADSL_LAN_ENABLE_IP2, (void *)&buffer, sizeof(buffer) );
	if (buffer[0])
		mib_get_s(MIB_ADSL_LAN_DHCP_POOLUSE, (void *)buffer, sizeof(buffer));
#else
	buffer[0] = 0;
#endif
	if (buffer[0] == 0) { // primary LAN
	if(!mib_get_s(MIB_ADSL_LAN_IP, (void *)&lanIp, sizeof(lanIp)) || !mib_get_s(MIB_ADSL_LAN_SUBNET, (void *)&lanMask, sizeof(lanMask)))
		return;
	}
	else { // secondary LAN
		if(!mib_get_s(MIB_ADSL_LAN_IP2, (void *)&lanIp, sizeof(lanIp)) || !mib_get_s(MIB_ADSL_LAN_SUBNET2, (void *)&lanMask, sizeof(lanMask)))
			return;
	}
...
// Kaohj
	...
	while (1)
	{
		...
		// Kaohj
#ifdef DHCPS_DNS_OPTIONS
		mib_get_s(MIB_DHCP_DNS_OPTION, (void *)&dnsopt, sizeof(dnsopt));
...
/*ping_zhang:20090310 END*/
			{
				...
				while (1) {
					...
					ret = getOneDhcpClient(&ptr, &fsize, ipAddr, macAddr, liveTime, &active_time);
					if (ret < 0)
						break;
					if (ret == 0)
						continue;
...
#endif  // #ifdef DHCPV6_ISC_DHCP_4XX
...
void filterAdd(unsigned char filterMode,struct in_addr *sip, struct in_addr *smask,unsigned short sfromPort,unsigned short stoPort,
	...
	syslogIPPortFilterEntry(filterEntry, 1, loginUser);
	return;
...
void filterdel(){
	...
	if(del==2)
	{
		syslog(LOG_INFO, "FW: %s del All IP Port Filtering rule.\n", loginUser);
	...
	else if(del==1)
	{
		...
		MIB_CE_IP_PORT_FILTER_T Entry;
		mib_chain_get(mibTblId, index-1, (void *)&Entry);
		syslogIPPortFilterEntry(Entry, 0, loginUser);
...
void MACFilterAdd(unsigned char act,unsigned char *src_macaddr,unsigned char *dst_macaddr,unsigned char dir){
	...
	if (src_macaddr[0]) {
		if (strlen(src_macaddr) != 12 || !rtk_string_to_hex(src_macaddr, macEntry.srcMac, 12) || !isValidMacAddr(macEntry.srcMac)) {
	...
	if (dst_macaddr[0]) {
		if (strlen(dst_macaddr) != 12 || !rtk_string_to_hex(dst_macaddr, macEntry.dstMac, 12) || !isValidMacAddr(macEntry.dstMac)) {
	...
	syslogMacFilterEntry(macEntry, 1, loginUser);
	return;
...
void MACFilterDel(){
	...
	if(del==2)
	{
		syslog(LOG_INFO, "FW: %s del All MAC Filtering rule.\n", loginUser);
	...
	else if(del==1)
	{
		...
		MIB_CE_MAC_FILTER_T Entry;
		mib_chain_get(mibTblId, index-1, (void *)&Entry);
		syslogMacFilterEntry(Entry, 0, loginUser);
                    ...
                    char *pszRemoteHost, unsigned short usExternalFromPort, unsigned short usExternalToPort)
{
	...
	syslogPortFwEntry(entry, 1, loginUser);
...
void delPortFW(){
	...
	if(del==2)
	{
		...
		syslog(LOG_INFO, "FW: %s del All Port Forwarding rule.\n", loginUser);
#endif
	...
	else if(del==1)
	{
		...
		MIB_CE_PORT_FW_T Entry;
		mib_chain_get(mibTblId, index-1, (void *)&Entry);
		syslogPortFwEntry(Entry, 0, loginUser);
	...
	while (1)
	{
		...
		switch( snum)
		{
			case 1://(1) Set
				...
				{
					syslog(LOG_INFO, "FW: %s %s Port Forwarding.\n", loginUser, vChar==1? "Enable":"Disable");
#if defined(APPLY_CHANGE)
					setupPortFW();
#endif
//Magician: Commit immediately
...
void URLBlkDel(int mibTblId)
{
	...
	if (del == 2){
		...
		if(mibTblId == MIB_URL_FQDN_TBL){
			...
			mib_chain_get(MIB_URL_FQDN_TBL, index-1, (void *)&Entry);
			syslogUrlFqdnEntry(Entry, 0, loginUser);
		}
		else{
			MIB_CE_KEYWD_FILTER_T Entry;
			mib_chain_get(MIB_KEYWD_FILTER_TBL, index-1, (void *)&Entry);
			syslogUrlKeywdEntry(Entry, 0, loginUser);
		}
		if(mib_chain_delete(mibTblId, index-1) != 1)
			printf("Delete chain record error!");
	}
		...
		switch (snum) {
			case 1:	//(1) Add
				...
				if (mibTblId == MIB_URL_FQDN_TBL) {
					...
					syslogUrlFqdnEntry(entryFQDN, 1, loginUser);
				}
				else if (mibTblId == MIB_KEYWD_FILTER_TBL) {
					...
					syslogUrlKeywdEntry(entryKeyword, 1, loginUser);
				}
				if (intVal == 0) {
...
void DomainBlkDel()
{
	...
	if (del == 2){
		...
		syslog(LOG_INFO, "FW: %s del All Domain Blocking rule.\n", loginUser);
	}
	else if (del == 1) {
		showDomainBlkTable();
		min=1; max=totalEntry+1;
...
#ifdef USE_LOGINWEB_OF_SERVER
		MIB_CE_DOMAIN_BLOCKING_T Entry;
		mib_chain_get(MIB_DOMAIN_BLOCKING_TBL, index-1, (void *)&Entry);
		syslogDomainBlockEntry(Entry, 0, loginUser);
#endif
...
//Magician: Commit immediately
			...
			case 2:	//(2) Add
				...
				syslogDomainBlockEntry(entry, 1, loginUser);
#if defined(APPLY_CHANGE)
				restart_domainBLK();
#endif
//Magician: Commit immediately
...
int dmzset(int enable, char *ip){
	...
	if(vChar==1) {
		...
		mib_get_s( MIB_ADSL_LAN_SUBNET,  (void *)&curSubnet, sizeof(curSubnet));
#ifdef CONFIG_SECONDARY_IP
		mib_get_s( MIB_ADSL_LAN_ENABLE_IP2, (void *)&ip2Enable, sizeof(ip2Enable) );
	...
	syslogDMZ(vChar, inet_ntoa(ipAddr), loginUser);
	return 1;
setErr_dmz:
...
void setDemilitarizedZone()
{
	...
	while (1)
	{
		...
		*/
		mib_get_s(MIB_DMZ_ENABLE, (void *)&state, sizeof(state));
...
void showconnlimitTable()
{
	...
	mib_get_s( MIB_CONNLIMIT_TCP, (void *)&tnum, sizeof(tnum));
	mib_get_s( MIB_CONNLIMIT_UDP, (void *)&unum, sizeof(unum));
...
//Magician: Commit immediately
			...
			case 2://(2) Add
				...
				min = 1;
				if (entry.protocol == 0)
					mib_get_s(MIB_CONNLIMIT_TCP,&max, sizeof(max));
				else
					mib_get_s(MIB_CONNLIMIT_UDP,&max, sizeof(max));
...
void setIGMPProxy()
{
	...
	while (1)
	{
		...
		printf("-------------------------------------------------------------------------\n");
		mib_get_s(MIB_IGMP_PROXY, (void *)&enabled, sizeof(enabled));
		printf("IGMP Proxy: %s\n", enabled? STR_ENABLE:STR_DISABLE);
		if (enabled) {
			mib_get_s(MIB_IGMP_PROXY_ITF, (void *)&proxy_if, sizeof(proxy_if));
			ifname[0] = 0;
			ifGetName(proxy_if, ifname, sizeof(ifname));
...
void setUPnP()
{
	...
	while (1)
	{
		...
		printf("-------------------------------------------------------------------------\n");
		mib_get_s(MIB_UPNP_DAEMON, (void *)&enabled, sizeof(enabled));
		printf("UPnP: %s\n", enabled? STR_ENABLE:STR_DISABLE);
		if (enabled) {
			mib_get_s(MIB_UPNP_EXT_ITF, (void *)&ext_if, sizeof(ext_if));
			ifname[0] = 0;
...
int RIPconfig(){
	...
	mib_get_s(MIB_RIP_ENABLE, (void *)&ripValGet, sizeof(ripValGet));
	if(ripValGet==1)
		strcpy(msgstr,"ON");
	else
		strcpy(msgstr,"OFF");
...
void setRIP()
{
	...
	while (1)
	{
		...
		printf("-------------------------------------------------------------------------\n");
		mib_get_s(MIB_RIP_ENABLE, (void *)&ripOn, sizeof(ripOn));
	...
	while (1)
	{
		...
		switch( snum)
		{
			case 1://(1) Set
				if (!check_access(SECURITY_SUPERUSER))
					break;
				mib_get_s(MIB_BRCTL_AGEINGTIME, (void *)&time, sizeof(time));
				...
				mib_get_s(MIB_BRCTL_STP, (void *)&stp, sizeof(stp));
				if(stp==1)
...
void showSNMP(){
	...
	if ( !mib_get_s( MIB_SNMPD_ENABLE, (void *)&enable, sizeof(enable))) {
		printf("Get snmpdEnable error!\n");
	}
	printf("SNMP:%s\n", (enable==1)? "enable":"disable");

	if ( !mib_get_s(MIB_SNMP_SYS_DESCR, (void *)str, sizeof(str))) {
		printf("Get snmpSysDescr mib error!\n");
	}
	printf("System Description:%s\n",str);

	if ( !mib_get_s(MIB_SNMP_SYS_CONTACT, (void *)str, sizeof(str))) {
		printf("Get mib error!\n");
	}
	printf("System Contact:%s\n",str);

	if ( !mib_get_s(MIB_SNMP_SYS_NAME, (void *)str, sizeof(str))) {
		printf("Get mib error!\n");
	}
	printf("System Name:%s\n",str);

	if ( !mib_get_s(MIB_SNMP_SYS_LOCATION, (void *)str, sizeof(str))) {
		printf("Get mib error!\n");
	}
	printf("System Location:%s\n",str);

	if ( !mib_get_s(MIB_SNMP_SYS_OID, (void *)str, sizeof(str))) {
		printf("Get mib error!\n");
	}
	printf("System Object ID:%s\n",str);

	if ( !mib_get_s(MIB_SNMP_TRAP_IP, (void *)&trap_ip, sizeof(trap_ip))) {
		printf("Get mib error!\n");
	}
	printf("Trap IP Address:%s\n",inet_ntoa(trap_ip));

	if ( !mib_get_s(MIB_SNMP_COMM_RO_ENC, (void *)str, sizeof(str))) {
		printf("Get mib error!\n");
	}
	printf("Community name (read-only):%s\n",str);

	if ( !mib_get_s(MIB_SNMP_COMM_RW_ENC, (void *)str, sizeof(str))) {
		printf("Get mib error!\n");
	}
...
void showOthers(){
	...
	if (vInt == DUMMY_IFINDEX)
		...
		mib_get_s( MIB_IPPT_LANACC, (void *)&laChar, sizeof(laChar));
		if (laChar)
...
//struct ippt_para setIPPT()
{
	...
	mib_get_s(MIB_IPPT_ITF, (void *)&old_ippt_itf, sizeof(old_ippt_itf));
	para->old_ippt_itf= old_ippt_itf;
	//printf("para.old_ippt_itf=%d\n", para.old_ippt_itf);

	// Get old IPPT Lease Time
	mib_get_s(MIB_IPPT_LEASE, (void *)&old_ippt_lease, sizeof(old_ippt_lease));
	para->old_ippt_lease= old_ippt_lease;
	//printf("para.old_ippt_lease=%d\n", para.old_ippt_lease);

	// Get old IPPT LAN access flag
	mib_get_s(MIB_IPPT_LANACC, (void *)&old_ippt_lanacc, sizeof(old_ippt_lanacc));
...
void showRADVD()
{
	...
	if (vChar!=0) {
		if ( !mib_get_s(MIB_V6_PREFIX_IP, (void *)str, sizeof(str))) {
			printf("Get Prefix_IP mib error!");
		}
		if ( !mib_get_s(MIB_V6_PREFIX_LEN, (void *)str2, sizeof(str2))) {
		...
		printf("\t{\n");
			// AdvValidLifetime
			if ( !mib_get_s(MIB_V6_VALIDLIFETIME, (void *)str, sizeof(str))) {
			...
			if ( !mib_get_s(MIB_V6_PREFERREDLIFETIME, (void *)str, sizeof(str))) {
		...
		switch( snum)
		{
			case 1://(1) Set
				...
				if ( snum == 1 ) {
					...
					prefix_mode = 0;
				mib_set(MIB_V6_RADVD_PREFIX_MODE, (void *)&prefix_mode);
...
static void addUser()
{
	char newuser[MAX_NAME_LEN], newpass[MAX_PASSWD_LEN], confpass[MAX_PASSWD_LEN], suName[MAX_NAME_LEN], usName[MAX_NAME_LEN];
	int entryNum, i, intVal;
	...
	if (strcmp(newpass, confpass) == 0) {
		strncpy(Entry.userPassword, newpass, MAX_PASSWD_LEN-1);
...
{
	char oldpass[MAX_PASSWD_LEN], newpass[MAX_PASSWD_LEN], confpass[MAX_PASSWD_LEN], oldMIBpass[MAX_PASSWD_LEN];
	...
	if (selnum == 1) {
   		if (!mib_get_s(MIB_SUSER_PASSWORD, (void *)oldMIBpass, sizeof(oldMIBpass)))
   		...
   		if (!mib_get_s(MIB_USER_PASSWORD, (void *)oldMIBpass, sizeof(oldMIBpass)))
		...
		if (strcmp(Entry.userPassword, oldpass) != 0) {
			printf("Old Password is not correct ! Plaese Input it again\n");
			return;
		}
	}
	...
	if (strcmp(newpass, confpass) == 0) {
		...
		printf("Confirmed Password is not correct ! Plaese Input it again\n");
}
#endif
...
void setPassword()
{
	...
	while(1)
	{
		...
		MSG_LINE;
		//printf("-------------------------------------------------------------------------\n");
		printf("This page is used to set the account to access the web server of %s    \n", ACCESS_PROTOCOL);
		printf("Router.                                                                  \n");
		MSG_LINE;
		//printf("-------------------------------------------------------------------------\n");
		...
		printf("\nIdx  UserName\n");
		printf("----------------\n");
		mib_get_s(MIB_SUSER_NAME, (void *)usName, sizeof(usName));
		printf ("1    %s\n", usName);
		mib_get_s(MIB_USER_NAME, (void *)usName, sizeof(usName));
		printf ("2    %s\n", usName);
		entryNum = mib_chain_total(MIB_ACCOUNT_CONFIG_TBL);
...
CONFIRM:
			...
			if ( strcmp(oldpass, oldMIBpass) != 0 ) {
				printf("Old Password is not correct ! Please Input it again\n");
				syslog(LOG_ERR, "Change password fail for %s change %s password via %s. Old Password is not correct !\n", loginUser, usName, ACCESSPROTOCOL[istelnet]);
				//goto CONFIRM;
				break;
			}
			...
			//printf("username=%s  newpass=%s  confpass=%s\n", username, newpass, confpass);
				...
				syslog(LOG_ERR, "Change password fail for %s change %s password via %s. Confirmed Password is not correct !\n", loginUser, usName, ACCESSPROTOCOL[istelnet]);
				//goto CONFIRM;
				break;
			}
			...
			syslog(LOG_INFO, "Change password successful for %s change %s password via %s\n", loginUser, usName, ACCESSPROTOCOL[istelnet]);
...
void aclDel()
{
	...
	if (del == 2){
		...
		mib_chain_get(MIB_ACL_IP_TBL, index-1, (void *)&Entry);
		syslogACLEntry(Entry, 0, loginUser);
	...
	while (1)
	{
		...
		case 2: // (2) Add
			...
			if (intVal == 0) {
				...
				printf("Error! Table Full.");
			syslogACLEntry(Entry, 1, loginUser);
			break;
		case 3: // (3) Delete
			aclDel();
			printWaitStr();
...
static void setTr069Config(int action)
{
	...
	if ( action == 1) {    // Enable/disable TR069
		...
		mib_get_s( CWMP_FLAG, (void *)&cwmp_flag, sizeof(cwmp_flag) );
		if ( cwmp_flag & CWMP_FLAG_AUTORUN )
			cwmp_flag_value = 1;
		else
			cwmp_flag_value = 0;
...
/*star:20100305 END*/
		...
		if (0==getInputUint("Inform (1) Disable (2) Enable :", &vInt, &vMin, &vMax))
			return;
		vChar = (vInt == 1) ? 0 : 1;
		mib_get_s( CWMP_INFORM_ENABLE, (void*)&old_vChar, sizeof(old_vChar));
		if(old_vChar != vChar){
			changeflag = 1;
		...
		if (vChar) {
			...
			if (0==getInputUint("Inform Interval :", &vInt, &vMin, &vMax))
				return;
			mib_get_s( CWMP_INFORM_INTERVAL, (void*)&informInterv, sizeof(informInterv));
		...
		if (0==getInputString("Path:", tmpStr, 32 ))
			return;
		mib_get_s( CWMP_CONREQ_PATH, (void *)strPath, sizeof(strPath));
		...
		if (0==getInputUint("Port:", &vInt, &vMin, &vMax))
			return;
		mib_get_s( CWMP_CONREQ_PORT, (void *)&old_port, sizeof(old_port));
		if ( vInt != old_port ) {
			changeflag = 1;
...
void setTR069() {
	...
	while (1)
	{
		...
		{
		case 1://(1) Show
			mib_get_s(CWMP_ACS_URL, (void *)tmpStr, sizeof(tmpStr));
			printf("ACS URL: %s\n", tmpStr);
			mib_get_s(CWMP_ACS_USERNAME, (void *)tmpStr, sizeof(tmpStr));
			printf("ACS Username: %s\n", tmpStr);
			mib_get_s(CWMP_ACS_PASSWORD, (void *)tmpStr, sizeof(tmpStr));
			...
			printf("\nConnection Request\n");
			mib_get_s(CWMP_CONREQ_USERNAME, (void *)tmpStr, sizeof(tmpStr));
			printf("Username: %s\n", tmpStr);
			mib_get_s(CWMP_CONREQ_PASSWORD, (void *)tmpStr, sizeof(tmpStr));
			printf("Password: %s\n", tmpStr);
...
#else
#ifdef USE_DEONET
		cmd_upload(fname, 0, st.st_size, 1, 1, 0);
#else
...
int SystemLogConfig(){
	...
	mib_get_s(MIB_SYSLOG, (void *)&getVal, sizeof(getVal));
	if(getVal==1)
		strcpy(msgstr,"ON");
	else
		strcpy(msgstr,"OFF");
...
#else
		mib_get( MIB_NTP_SERVER_ID, (void *)&server_id);
		server_id |= 0x01;
		mib_set( MIB_NTP_SERVER_ID, (void *)&server_id);
#endif
...
void showDoS()
{
	...
	mib_get_s(MIB_DOS_BLOCK_TIME, (void *)&uIntValue, sizeof(uIntValue));
...
void changeDoS()
{
	...
	if (sel == 1) {
		...
		mib_get_s(MIB_DOS_SYSSYN_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_SYSFIN_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_SYSUDP_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_SYSICMP_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_PIPSYN_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_PIPFIN_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_PIPUDP_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_PIPICMP_FLOOD, (void *)&floodCount, sizeof(floodCount));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
		...
		mib_get_s(MIB_DOS_BLOCK_TIME, (void *)&blockTimer, sizeof(blockTimer));
		if (retVal == 0) {
			printf("Invalid value!\n");
			printWaitStr();
			return;
...
#ifdef CONFIG_USER_PPPOMODEM
	{
		...
		if (mib_chain_get(MIB_WAN_3G_TBL, 0, p)) {
			if (p->enable) {
				...
				get_net_device_stats(ifname, &nds);
				printf("3G_%s\t%llu\t%llu\t%llu\t%llu\t%llu\t%llu\n",
...
#if 0
	mib_get_s(MIB_SUSER_NAME, (void*)suusr, sizeof(suusr));
	mib_get_s(MIB_SUSER_PASSWORD, (void*)supass, sizeof(supass));
#else
	// use HW setting first
	mib_get_s(MIB_HW_E8BDUSER_NAME, (void*)suusr, sizeof(suusr));
	if(suusr[0] == '\0')
	{
		mib_get_s(MIB_E8BDUSER_NAME, (void*)suusr, sizeof(suusr));
		mib_get_s(MIB_E8BDUSER_PASSWORD, (void*)supass, sizeof(supass));
	}
	else
		mib_get_s(MIB_HW_E8BDUSER_PASSWORD, (void*)supass, sizeof(supass));
#endif
...
#endif
#define CLI_SH_PASSWD "deonet1234!@"
int checkShellPasswd(void)
{
	char passwd[MAX_NAME_LEN];
	printf("Please input Password:");
	if (getInputStrNoDisplay(passwd, sizeof(passwd) - 1, NULL) == 0) return 0;
	if (passwd[strlen(passwd) - 1] == '\n')
		passwd[strlen(passwd) - 1] = 0;

	if(!strcmp(passwd, CLI_SH_PASSWD))
	{
		return 1;
	}
	return 0;
...
int main(int argc, char *argv[])
{
	...
	FILE *pp = popen("export | grep DISPLAY", "r");
	char newline[256];
		
	if(pp)
	{
		if(fgets(newline, 256, pp) != NULL){
			istelnet = 1; //telnet or ssh
		}
		else{
			istelnet = 0; //console
		}
