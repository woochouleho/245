...
#define INTERFACE_IP_MIB_ID MIB_LAN_RGMII_IP
#else
#error "Please define CONFIG_SECONDARY_IP."
#endif
...
int rt_qcsapi_get_sta_rssi(char *ifname, unsigned int index, unsigned int *rssi)
{
	int ret=0;
	const char *host;
	CLIENT *clnt=NULL;
	...
	if((ret = qcsapi_wifi_get_rssi_per_association(ifname, index, rssi))!=0)
		printf("%s %d: failed %d \n", __FUNCTION__,__LINE__, ret);
...
int rt_qcsapi_get_sta_snr(char *ifname, unsigned int index, unsigned int *snr)
{
	int ret=0;
	const char *host;
	CLIENT *clnt=NULL;
	...
	if((ret = qcsapi_wifi_get_snr_per_association(ifname, index, (int *)snr))!=0)
		printf("%s %d: failed %d \n", __FUNCTION__,__LINE__, ret);
...
int setupWLan_qtn(char *ifname, int index)
{
	...
	if(index==0){
		...
		mib_get_s(MIB_WLAN_BEACON_INTERVAL, &sInt, sizeof(sInt));
		...
		mib_get_s(MIB_WLAN_CHAN_NUM, &vChar, sizeof(vChar));
		mib_get_s(MIB_WLAN_AUTO_CHAN_ENABLED, &auto_chan, sizeof(auto_chan));
		...
		mib_get_s(MIB_WLAN_BAND, &vChar, sizeof(vChar));
		...
		mib_get_s(MIB_WLAN_DTIM_PERIOD, &vChar, sizeof(vChar));
		vInt = (unsigned int)vChar;
		qcsapi_wifi_set_dtim(ifname, vInt);

		mib_get_s(MIB_WLAN_SHORTGI_ENABLED, &vChar, sizeof(vChar));
		qcsapi_wifi_set_option(ifname, qcsapi_short_GI, (int)vChar);

		mib_get_s(MIB_WLAN_RTS_THRESHOLD, &sInt, sizeof(sInt)); 
		vInt = (unsigned int) sInt;
	...
	if(Entry.encrypt == WIFI_SEC_NONE){ 
		...
		if(Entry.enable1X){
			...
			mib_get_s(INTERFACE_IP_MIB_ID, &ip_addr, sizeof(ip_addr));
