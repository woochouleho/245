...
int init_cgi(request * req)
{
	...
	//check cgi exist or not
	if(dirmaker)
		snprintf(path, sizeof(path),"%s%s",dirmaker,req->pathname);
	else
		snprintf(path, sizeof(path),"%s%s","/home/httpd/",req->pathname);
	...
	if (!child_pid) {			/* 0 == child */
		...
		if (req->is_cgi) {
			char *aargv[ARGC_MAX + 1], *tmp_dupstr;
			tmp_dupstr = create_argv(req, aargv);
