...
int init_form(request * req)
{
	...
	if(fstat(req->post_data_fd, &statbuf) != 0)
...
int init_get2(request * req)
{
	...
	if(strstr(req->request_uri,".asp")==NULL && strstr(req->request_uri,"share.js")==NULL
		...
		&& strstr(req->request_uri,"menu_dir.js")==NULL
		&& strstr(req->request_uri,"index_user.html")==NULL
#endif
...
//	req->last_modified = statbuf.st_mtime;
...
#else
	{
		...
		while(1)
		{
			/* Note: data_mem is memory mapped file (mmap), we should not use strstr() to search for text;
			 *	Use memmem() instead. */
			bob=(uintptr_t)last_right - (uintptr_t)req->data_mem;
			haystack_len = req->filesize - bob;
			if (haystack_len <= 0)
				break;
			left=memmem(last_right, haystack_len, "<%", strlen("<%"));
			if (left!=NULL) {
				bob=(uintptr_t)left-(uintptr_t)last_right;
				haystack_len-=bob;
				right=memmem(left, haystack_len, "%>", strlen("%>"));
			}
			...
			if((left!=NULL) && (right!=NULL))
			{
				...
				if(bob>=0)
				{
					...
					last_right=right+2;
					req->buffer_end += bob;
					req->filepos += (bob+(uintptr_t)last_right-(uintptr_t)left);
			...
			else
			{
				...
				bob=(uintptr_t)req->data_mem+req->filesize-(uintptr_t)last_right;
				while((bob+req->buffer_end)>req->max_buffer_size)
				{
					int ret;
					ret=allocNewBuffer(req);
...
int get_dir(request * req, struct stat *statbuf)
{
	...
	if (data_fd != -1) {		/* user's index file */
		strncat(req->request_uri, directory_index,sizeof(req->request_uri)-strlen(req->request_uri)-1); /* for mimetype */
		if(fstat(data_fd, statbuf) != 0)
			printf("fstat failed: %s %d\n", __func__, __LINE__);
		return data_fd;
	}
