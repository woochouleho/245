...
function showDhcpSvr(ip)
{
	...
	if (document.dhcpd.dhcpdenable[0].checked == true)
		...
		html +=
			...
			'<input class="link_bg" type="button" value="<% multilang(LANG_MAC_BASED_ASSIGNMENT); %>" name="macIpTbl" onClick="macIpClick(\'/macIptbl.asp\')">'+
			'<input class="link_bg" type="button" value="<% multilang(LANG_DEVICE); %><% multilang(LANG_IP_RANGE); %>" name="clientIpTbl" onClick="clientIpClick(\'/devicetbl.asp\')">'+
				'</div>';
