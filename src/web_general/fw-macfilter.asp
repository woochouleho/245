...
//	return true;
  if (document.formFilterAdd.srcmac.value=="" && document.formFilterAdd.dstmac.value=="") {	
    ...
    if (isAttackXSS(document.formFilterAdd.desc.value)) {
        alert("사용할 수 없는 특수 문자가 포함되어 있습니다.");
        return false;
    }
...
<form action=/boaform/admin/formFilter method=POST name="formFilterDefault" style="display:none;">
...
<table>
	...
	<tr>
		...
		<td>
			<select name=port>
				<option select value=2>LAN1</option>
				<option value=3>LAN2</option>
				<option value=4>LAN3</option>
				<option value=5>LAN4</option>
			</select>
		</td>
	</tr>
	<tr>
		<th><% multilang(LANG_DESCRIPTION); %>: </th>
	    <td>
	        <input type="text" name="desc" size="40" maxlength="40" ></input>
		</td>
	</tr>
	</table>
</div>	
