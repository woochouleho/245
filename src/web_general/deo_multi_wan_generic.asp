...
var ipv6_mode= '<% getInfo("ipv6_mode"); %>';
...
function applyCheck(obj)
{
	...
	{
        if (document.ethwan.dns1.value == document.ethwan.dns2.value)
        {
			alert('<% multilang(LANG_INVALID_DNSV4_2_IP_ADDRESS_VALUE); %>');
...
function dnsModeClicked()
{
	...
	{
        document.ethwan.dns1.value = "219.250.36.130";
        disableTextField(document.ethwan.dns1);
...
function on_linkchange(itlk, isinit)
{
	var pmchkpt = document.getElementById("tbl_pmap");
...
function on_ctrlupdate(isinit)
{
	document.getElementById("multicast_vid_tr").style.display = "none";
	document.getElementById('enblVlan').style.display = 'none';
	document.getElementById('vlanId').style.display = 'none';
	document.getElementById('mark802').style.display = 'none';
	document.getElementById('channelMode').style.display = 'none';
	document.getElementById('enblNAPT').style.display = 'none';
	document.getElementById('adminStatus').style.display = 'none';
	document.getElementById('IGMPProxy_show').style.display = 'none';
	document.getElementById('MLDProxy_show').style.display = 'none';
	document.getElementById('tbprotocol').style.display = 'none';
	document.getElementById('lknameTh').style.display = 'none';
	document.getElementById('tbmtu').style.display = 'none';
	document.getElementById('tbmtu_pppoe').style.display = 'none';
	document.getElementById('tbl_ppp').style.display = 'none';
	document.getElementById('div_pmap').style.display = 'none';
	document.getElementById('ipUnnumberd').style.display = 'none';
	with ( document.forms[0] )
	{
        if (chIntfMode[0].checked) {
			curlink = links[0];
		}
		else{
			curlink = links[1];
		}
		on_linkchange(curlink, isinit);
        if (chIntfMode[0].checked) {
...
function on_init()
{
	...
	with ( document.forms[0] )
	{
		...
		if(wan_nat_mode == 1) 
		{
			lkname.selectedIndex = 0;
			chIntfMode[0].checked = true;
			chIntfMode[1].checked = false;
		}
		else
		{
			lkname.selectedIndex = 1;
			chIntfMode[0].checked = false;
			chIntfMode[1].checked = true;
		}
		on_ctrlupdate(1);
		document.getElementById('ethwan').style.display = '';
	}
}
...
<form action=/boaform/admin/formWanEth method=POST id="ethwan" name="ethwan" style="display:none">
...
<table>
	<tr>
		...
		<th><% multilang(LANG_WAN_MODE); %>:</th>
		<th id="lknameTh"><select id="lkname" name="lkname" onChange="on_ctrlupdate(0)" size="1"></th>
		<td><input type=radio value=0 name="chIntfMode" onClick="on_ctrlupdate(0)"><% multilang(LANG_NAT); %>
			<input type=radio value=1 name="chIntfMode" onClick="on_ctrlupdate(0)" checked><% multilang(LANG_BRIDGE); %>
		</td>
	</tr>
	<tr id=enblVlan>
...
<div class="btn_ctl">
<input type="hidden" value="/admin/deo_multi_wan_generic.asp" name="submit-url">
