...
var channel_list_6g =[[],
					  ...
					  129,133,137,141,145,149,153,157,161,165,169,173,177,181,185,189,193,197,201,205,209,213,217,221,225,229,233], /* FCC */
					  [1,5,9,13,17,21,25,29,33,37,41,45,49,53,57,61,65,69,73,77,81,85,89,93],
					  [1,5,9,13,17,21,25,29,33,37,41,45,49,53,57,61,65,69,73,77,81,85,89,93], /* ESTI (CE) */
					  [36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140],
					  [36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140],
					  [36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140],
					  [36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140],
...
function saveChanges(obj)
{
	...
	if ((document.wlanSetup.repeaterEnabled.checked) && (document.wlanSetup.repeaterSSID.value=="")) {
		alert("<% multilang(LANG_VXD_SSID_CANNOT_BE_EMPTY); %>");
...
*/
	...
	if (document.wlanSetup.wlanDisabled.value != "ON") {
		...
		if(!(band & checked_band) && (checked_band!=0)){
			alert("<% multilang(LANG_THE_DATA_RATE_IS_INVALID_FOR_CURRENT_BAND); %>");
...
*/
	...
	if(document.wlanSetup.wlan6gSupport.checked == true){
		if(<% getVirtualIndex("encrypt","0"); %> != 16)
			alert("Please goto Security page to change encryption!");
	}
	obj.isclick = 1;
...
*/	
	...
	if(((parseInt(band_value, 10)+1) & 8) || ((parseInt(band_value, 10)+1) & 128)){		/* 11n */
		if(bound == 0)
			adjust_chan = 0;
		if(bound == 1)
			adjust_chan = 1;
...
<div class="data_common data_common_notitle">
	<table>
		...
		<tr id="optionfor6g" style="display:none">
			<th colspan=2>
				<input type="checkbox" name="wlan6gSupport" onClick="updateChan(); updateBand(document.wlanSetup);">&nbsp;&nbsp;<% multilang(LANG_ENABLE_6GHZ_SUPPORT); %>
			</th>
		</tr>
		<tr id="optionforband">
	...
	<table>
		...
		<tr>
			...
			<td>
				...
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<input type='submit' name="suggest_chan_enable" value='<% multilang(LANG_SWITCH); %>' onClick="return Change_channel(this)">
			</td>
		</tr>
		...
		<script>
         if(isCMCCSupport==1){	
		  document.write('<tr>');			
		  document.write('<th width="26%"><% multilang(LANG_SSID_PRIORITY); %>:</th>');		
	  ...
	  <tr id="repeater_SSID" style="display:none">
	      <th width="26%">SSID of Extended Interface:&nbsp;&nbsp;</th>
