...
function removeOptionsByValue(selectElement, value) {
...
function on_ctrlupdate()
{
	...
	var addrModeSelect = document.getElementById('AddrMode');
	removeOptionsByValue(addrModeSelect, '1'); // Stateless DHCPv6(SLAAC)
	removeOptionsByValue(addrModeSelect, '16'); // Stateful DHCPv6
...
function on_init()
{
	...
	with ( document.forms[0] )
	{
		...
		if(ipv6_mode == 0)
		{
			...
			ipv6Enable[1].checked = false;
		}
		else
		{
			ipv6Enable[0].checked = false;
...
<form action=/boaform/admin/formWanEth method=POST name="ethwan" id="ethwan" style="display:none">
...
<table>
	<tr>
		...
		<th id="lknameTh"><select id="lkname" name="lkname" onChange="on_ctrlupdate()" size="1"></th>
		<td><input type=radio value=0 name="ipv6Enable" onClick="on_ctrlupdate()"><% multilang(LANG_DISABLE); %>
			<input type=radio value=1 name="ipv6Enable" onClick="on_ctrlupdate()" checked><% multilang(LANG_ENABLE); %>
		</td>
	</tr>
	<tr id=enblVlan>
