...
void set_server_port(int port) {
	...
	{
		server_port = port;
		fixup_server_port = port;
	}
}

void chcek_server_port(void) {
	if(fixup_server_port > 0 && server_port != fixup_server_port)
	{
		server_port = fixup_server_port;
	}
}
...
void read_config_files(void)
{
  ...
  if (!server_name) {
	...
	// if unify the server port by command line, 
	// need check and override server_port if it different with server_port.
	chcek_server_port();
}
