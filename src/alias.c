...
int translate_uri(request * req)
{
	...
	if(!strncasecmp(req_urip, "http://", 7) && (p = strchr(req_urip+7, '/')))
	{
		len = strlen(req_urip) - (p-req_urip);
		if(len > 0){
			memcpy(req_urip, p, len);
			req_urip[len] = '\0';
			//printf("===> %s\n", req_urip);
		}
	}
	else if(!strncasecmp(req_urip, "https://", 8) && (p = strchr(req_urip+8, '/')))
	{
		len = strlen(req_urip) - (p-req_urip);
		if(len > 0){
			memcpy(req_urip, p, len);
	...
	if(strcmp(req_urip, "/admentor/admin/") == 0) {
  ...
  while (current) {
	...
	if(strstr(req->pathname,"/jembenchtest"))
	{
		req->is_cgi = CGI;
		return 1;
	}
...
#endif
		//WebInspect_10.2 expects error code 404
		#if 1
		    send_r_not_found(req);
		#else
